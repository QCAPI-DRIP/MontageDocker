...
PROFILES = [
    Profile(
        ...
        #------------------------------------------------------------------------------------------------------------------------
        OutputTemplate('stats',PlainTextFormat,'Name of table for output statistics (time of each reprojection, or error messages).',
...
#
COMMAND = WEBSERVICEDIR + "/mProjExec_wrapper.py $DATAFILE $STATUSFILE $OUTPUTDIRECTORY"

#Or for the shell variant:
#COMMAND = WEBSERVICEDIR + "/mProjExec_wrapper.sh $STATUSFILE $INPUTDIRECTORY $OUTPUTDIRECTORY $PARAMETERS"
...
PARAMETERS =  [
    ('Switches', [
        BooleanParameter(id='q',name='q',description='Use mProjectQL for reprojections. Much faster but not quite flux-conserving.'),
        BooleanParameter(id='d',name='d',description='Turns on debugging'),      
        BooleanParameter(id='e',name='e',description='Flag indicating output image should exactly match the FITS header template, and not crop off blank pixels'),  
        BooleanParameter(id='X',name='X',description='Force reprojection of whole images, even if they exceed the area of the FITS header template'),         
        StringParameter(id='b',name='b',description='Ignore border width of pixels around edge of images',maxlength=255),  
        StringParameter(id='r',name='r',description='Allows restart at record number restartrec, if mProjExec exits upon an error',maxlength=255),        
        StringParameter(id='x',name='x',description='Turn on flux rescaling (e.g. magnitude zero point correction): scaleColumn is the name of a column in images.tbl which contains scale information.',maxlength=255),         
