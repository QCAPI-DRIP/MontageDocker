...
PROFILES = [
    Profile(
        InputTemplate('directory', PlainTextFormat,"input FITS files",
            StaticParameter(id='encoding',name='Encoding',description='The character encoding of the file', value='utf-8'), 
            extension='.fits',
            multi=True #set unique=True if the user may only upload a file for this input template once. Set multi=True if you the user may upload 
        ),        
        #InputTemplate('fieldlistfile', PlainTextFormat,"Used to specify a fieldlist, which will list additional keywords to be read from the FITS headers and included in the output table. Fieldlists should specify the keyword name, type (int,char,double), and size. Example: example.fieldlist.",
            #StaticParameter(id='encoding',name='Encoding',description='The character encoding of the file', value='utf-8'), 
            #extension='.fieldlist',
            #unique=True #set unique=True if the user may only upload a file for this input template once. Set multi=True if you the user may upload 
        #),
        #InputTemplate('imglist', PlainTextFormat,"mImgtbl will only process files with names specified in table imglist, ignoring any other files in the directory. Example: example.imglist.",
            #StaticParameter(id='encoding',name='Encoding',description='The character encoding of the file', value='utf-8'), 
            #extension='.imglist',
            #unique=True #set unique=True if the user may only upload a file for this input template once. Set multi=True if you the user may upload 
        #),           
        ##------------------------------------------------------------------------------------------------------------------------
        OutputTemplate('images',PlainTextFormat,'Output table contains metadata information from all readable FITS files in the directory specified by directory. count is the number of records in the table, and badfits is the number of files for which FITS metadata could not be extracted.',
...
#
COMMAND = WEBSERVICEDIR + "/mImgtbl_wrapper.py $DATAFILE $STATUSFILE $OUTPUTDIRECTORY"

#Or for the shell variant:
#COMMAND = WEBSERVICEDIR + "/mImgtbl_wrapper.sh $STATUSFILE $INPUTDIRECTORY $OUTPUTDIRECTORY $PARAMETERS"
