...
To develop your webservice, edit your service configuration file ``/root/mProject/mProject/mProject.py`` , the host specific
configuration file ``/root/mProject/mProject/mProject.$HOSTNAME.yml``, and your system wrapper script
``/root/mProject/mProject/mProject_wrapper.py`` (or ``/root/mProject/mProject/mProject_wrapper.sh`` if you prefer to use a simple shell script
rather than Python).
...
Also don't forget to edit the metadata in the setup script ``/root/mProject/setup.py`` and run ``python setup.py install`` to install the webservice after you made any changes (the start scripts provided will do so automatically for you).
...
such as Apache (with mod_uwsgi_proxy), or nginx. A uwsgi configuration has been generated (mProject.$HOSTNAME.ini); it is specific
to the host you deploy the webservice on. This in turn loads the wsgi script (mProject.wsgi), which loads your webservice.
...
We *strongly* recommend you to put /root/mProject and everything inside under version control (e.g. git).
